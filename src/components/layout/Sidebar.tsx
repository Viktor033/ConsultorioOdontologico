            ...
            >
                ...
                <nav style={{ flex: 1, padding: '1.5rem 0.75rem', display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
                    {menuItems.map((item) => {
                        ...
                        return (
                            <Link key={item.id} href={item.path} style={{ textDecoration: 'none' }} onClick={() => setIsOpenMobile?.(false)}>
                                <motion.div
                                    whileHover={{ x: 4 }}
                                    style={{
                                        display: 'flex',
                                ...
                                >
                                    ...
                                    {(!isCollapsed || (typeof window !== 'undefined' && window.innerWidth <= 1024)) && <span>{item.label}</span>}
                                </motion.div>
                            </Link>
                        );
                    })}
                ...
                <div style={{ padding: '1.5rem 0.75rem', borderTop: '1px solid var(--bg-main)' }}>
                    {/* User info */}
                    {user && (!isCollapsed || (typeof window !== 'undefined' && window.innerWidth <= 1024)) && (
                        <div style={{
                    ...
                    <Link href="/configuracion" style={{ textDecoration: 'none' }} onClick={() => setIsOpenMobile?.(false)}>
                        <div style={{
                            ...
                            {(!isCollapsed || (typeof window !== 'undefined' && window.innerWidth <= 1024)) && <span>Configuración</span>}
                        </div>
                    </Link>
                    <div
                        onClick={() => { logout(); router.replace('/login'); setIsOpenMobile?.(false); }}
                        style={{
                            display: 'flex',
                            alignItems: 'center',
                            gap: '1rem',
                    ...
                    >
                        <LogOut size={22} />
                        {(!isCollapsed || (typeof window !== 'undefined' && window.innerWidth <= 1024)) && <span>Cerrar Sesión</span>}
                    </div>
                </div>
